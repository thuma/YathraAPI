...
try:
    response = http_client.fetch('https://raw.githubusercontent.com/thuma/Transit-Stop-Identifier-Conversions-Sweden/master/dalatrafik-gtfs.csv')
...
class Handler(tornado.web.RequestHandler):
	...
	def get(self):
		...
		req +='&EU_Spirit=False'
		req +='&TNSource=DALARNAS'
		req +='&SupportsScript=True'
		req +='&Language=se'
		req +='&VerNo=7.1.1.2.0.38p3'
		req +='&Source=querypage_adv'
		req +='&MapParams='
		self.url = 'http://thuma.github.io/gettopost.html#http://193.45.213.123/dalarna/v2/querypage_adv.aspx&&&'+req
		self.myhttprequest = tornado.httpclient.HTTPRequest('http://193.45.213.123/dalarna/v2/querypage_adv.aspx', method='POST', headers=None, body=req) 
