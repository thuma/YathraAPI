...
try:
    response = http_client.fetch('https://github.com/thuma/Transit-Stop-Identifier-Conversions-Sweden/raw/master/svenskbuss.csv')
...
class Handler(tornado.web.RequestHandler):
	...
	def get(self):
		...
		self.myhttprequest = tornado.httpclient.HTTPRequest('http://www.svenskabuss.se/ajax/get_travel_plans?date_outbound='+self.get_argument('date')+'&date_return='+self.get_argument('date')+'&from_stop_id='+fromid+'&to_stop_id='+toid+'&travellers%5B96%5D=1', method='GET') 
	...
	def searchdone(self, response):
		...
		try: 
			...
			outdata['url'] = 'http://thuma.github.io/gettopost.html#http://www.svenskabuss.se/&&&from='+stops[self.get_argument('from')]+'&to='+stops[self.get_argument('to')]+'&date_outbound='+self.get_argument('date')+'&date_return='+self.get_argument('date')+'&type_96=1&type_98=0&type_101=0&type_99=0&type_100=0&btn_continue=S%F6k+avg%E5ngar'
